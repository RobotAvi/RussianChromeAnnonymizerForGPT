...
<img width="1797" height="992" alt="image" src="https://github.com/user-attachments/assets/dc33394b-d3fd-49d1-9862-b2943647035b" />
...
1. Скачайте файл последней версии расширения, например, RussianChromeAnonymizer_v1.0.0.zip из папки [releases](https://github.com/RobotAvi/RussianChromeAnnonymizerForGPT/tree/main/releases).
2. Распакуйте этот zip-архив в отдельную папку на вашем компьютере. Убедитесь, что внутри есть файл manifest.json. Иногда при распаковке создаётся несколько вложенных папок — выберите именно ту, где находится этот файл.
3. Откройте браузер Google Chrome.
4. В адресной строке введите chrome://extensions/ и нажмите Enter.
5. В правом верхнем углу страницы включите переключатель "Режим разработчика".
6. Нажмите кнопку "Загрузить распакованное расширение".
7. В появившемся окне укажите папку, в которую вы распаковали файлы расширения (ту, где находится manifest.json).
8. Расширение установится и появится в списке. Теперь им можно пользоваться.

**Если при выборе папки появляется ошибка "не найден манифест", значит вы выбрали не ту папку — пройдите глубже, пока не найдёте manifest.json**
